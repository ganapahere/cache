...
extern cpuid_asm(unsigned char *, unsigned char *, unsigned char *, unsigned char *);
...
struct _cpuid_str cpuid_str[] = {
    ...
    {0x63, "TLB Data TLB: 2 MByte or 4 MByte pages, 4-way set associative, 32 entries and a separate array with 1 GBytes, 4-way set associative, 4 entries"},
...
int main()
{
    ...
    int i, j;
    unsigned int *ptr = (unsigned int *)&reg[0];

    cpuid_asm(&reg[0], &reg[4], &reg[8], &reg[12]);
    ...
    for (i = 0; i < 16; i++) {
        ...
        for (j = 0 ; j < 223; j++) {
            if (reg[i] == cpuid_str[j].idx) {
                printf("0x%02x:\t%s\n", cpuid_str[j].idx, cpuid_str[j].info);
                break;
            }
        }
    }
